      ...
      initial_pose_received_(false)
{
    ...
    pose_update_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
        "/pose_update", 10, std::bind(&DMapLocalizationNode::pose_update_callback, this, std::placeholders::_1));
    ...
    for (int y = 0; y < distance_map_.rows(); ++y)
    {
        for (int x = 0; x < distance_map_.cols(); ++x)
        {
            ...
            else
            {
                ...
                int value = static_cast<int>( (1.0f - (distance / max_distance)) * 100.0f );
...
void DMapLocalizationNode::pose_update_callback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Initial pose received: x=%.2f, y=%.2f, yaw=%.2f",
...
void DMapLocalizationNode::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
    std::lock_guard<std::mutex> lock(pose_mutex_);
...
void DMapLocalizationNode::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
    if (!distance_map_computed_ || !initial_pose_received_)
        return;

    perform_localization(msg);
}

void DMapLocalizationNode::perform_localization(const sensor_msgs::msg::LaserScan::SharedPtr scan)
{
    std::lock_guard<std::mutex> lock(pose_mutex_);
    ...
    for (int iter = 0; iter < max_iterations; ++iter)
    {
        ...
        for (const auto& point_laser : scan_points)
        {
            ...
            point_world(0) = pose_estimate(0) + cos_theta * point_laser(0) - sin_theta * point_laser(1);
            point_world(1) = pose_estimate(1) + sin_theta * point_laser(0) + cos_theta * point_laser(1);

            // Convert world coordinates to map indices
            int map_x = static_cast<int>((point_world(0) - map_->info.origin.position.x) / map_->info.resolution);
            int map_y = static_cast<int>((point_world(1) - map_->info.origin.position.y) / map_->info.resolution);
                ...
                map_y >= 1 && map_y < static_cast<int>(map_->info.height) - 1)
            {
                ...
                float grad_x = (distance_map_(map_y, map_x + 1) - distance_map_(map_y, map_x - 1)) / (2 * map_->info.resolution);
                float grad_y = (distance_map_(map_y + 1, map_x) - distance_map_(map_y - 1, map_x)) / (2 * map_->info.resolution);
                ...
                J(0, 0) = grad_x;
                J(0, 1) = grad_y;
                J(0, 2) = grad_x * (-sin_theta * point_laser(0) - cos_theta * point_laser(1)) +
        ...
        // Normalize angle
        pose_estimate(2) = std::atan2(std::sin(pose_estimate(2)), std::cos(pose_estimate(2)));
    ...
    T_map_robot_new.setOrigin(tf2::Vector3(current_pose_(0), current_pose_(1), 0.0));
...
{
    // Publish localization_pose as before
    ...
    // Use T_map_odom_ to send the transform
